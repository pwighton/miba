...
SUB_LIST=/autofs/vast/gerenuk/pwighton/pet/cox1-proc-mapping.txt

SUB_NAME=$(cat $SUB_LIST | sed -n "${SLURM_ARRAY_TASK_ID}p" | awk '{print $1}')
TIMEPOINT=$(cat $SUB_LIST | sed -n "${SLURM_ARRAY_TASK_ID}p" | awk '{print $2}')
OUT_DIR=$(cat $SUB_LIST | sed -n "${SLURM_ARRAY_TASK_ID}p" | awk '{print $3}')
OUT_DIR_FULL=${SUBJECTS_DIR}/${SUB_NAME}/${OUT_DIR}
